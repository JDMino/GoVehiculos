  ...
  if (loading) return (
    <div className="min-h-screen flex items-center justify-center bg-slate-50">
      <div className="text-center">
        ...
        <p className="text-slate-500 font-medium">Cargando datos del vehiculo...</p>
      </div>
    </div>
  );
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                <div className="flex items-center gap-3 mb-1">
                  ...
                  <span className={`inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-semibold ring-1 ring-inset ${getEstadoBadgeStyles(form.estado)}`}>
                ...
                <div className="flex items-center gap-3 text-slate-300 text-sm">
                  <span>ID #{id}</span>
                  <span>&bull;</span>
                  <span>Ano {vehiculoInfo?.anio}</span>
                  <span>&bull;</span>
                  <span className="font-mono bg-white/10 px-2 py-0.5 rounded">{vehiculoInfo?.patente}</span>
                </div>
              </div>
            </div>
            {!form.activo && (
              <div className="flex items-center gap-2 px-4 py-2 bg-red-500/20 border border-red-400/30 rounded-xl">
                ...
                <span className="text-red-200 font-medium text-sm">Vehiculo Inactivo</span>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        {form.estado === 'mantenimiento' && (
          <div className="mb-6 p-4 bg-amber-50 border border-amber-200 rounded-2xl flex items-start gap-3">
            ...
            <div>
              <p className="font-semibold text-amber-800">Vehiculo en mantenimiento</p>
              <p className="text-amber-700 text-sm">Este vehiculo no esta disponible para alquiler hasta que se complete el mantenimiento.</p>
            </div>
          </div>
        )}
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  <label className="text-sm font-medium text-slate-700">Estado de Operacion</label>
                  <select 
                    name="estado" 
                    value={form.estado} 
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-slate-700">Condicion Mecanica</label>
                  <select 
                    name="estadoMecanico" 
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-slate-700">Kilometraje Actual</label>
                ...
                <div className="space-y-2">
                  <label className="text-sm font-medium text-slate-700">Ubicacion / Sucursal</label>
                  <div className="relative">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <ShieldCheck className={`h-5 w-5 ${form.seguroVigente ? 'text-emerald-600' : 'text-slate-400'}`} />
                    <div className={`w-10 h-6 rounded-full p-1 transition-colors ${form.seguroVigente ? 'bg-emerald-500' : 'bg-slate-300'}`}>
                      <div className={`w-4 h-4 rounded-full bg-white shadow transition-transform ${form.seguroVigente ? 'translate-x-4' : 'translate-x-0'}`} />
                    </div>
                  </div>
                  <p className="font-semibold text-slate-900 text-sm">Seguro Vigente</p>
                  <p className="text-xs text-slate-500 mt-0.5">Poliza de seguro activa</p>
                </button>

                {/* Documentacion Vigente Toggle */}
                <button
                  type="button"
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    <FileCheck className={`h-5 w-5 ${form.documentacionVigente ? 'text-emerald-600' : 'text-slate-400'}`} />
                    <div className={`w-10 h-6 rounded-full p-1 transition-colors ${form.documentacionVigente ? 'bg-emerald-500' : 'bg-slate-300'}`}>
                      <div className={`w-4 h-4 rounded-full bg-white shadow transition-transform ${form.documentacionVigente ? 'translate-x-4' : 'translate-x-0'}`} />
                    </div>
                  </div>
                  <p className="font-semibold text-slate-900 text-sm">Doc. Vigente</p>
                  <p className="text-xs text-slate-500 mt-0.5">Papeles al dia</p>
                </button>

                {/* Vehiculo Activo Toggle */}
                <button
                  type="button"
                ...
                >
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <div className={`w-10 h-6 rounded-full p-1 transition-colors ${form.activo ? 'bg-emerald-500' : 'bg-red-500'}`}>
                      <div className={`w-4 h-4 rounded-full bg-white shadow transition-transform ${form.activo ? 'translate-x-4' : 'translate-x-0'}`} />
                    </div>
                  </div>
                  <p className="font-semibold text-slate-900 text-sm">Vehiculo Activo</p>
                  <p className="text-xs text-slate-500 mt-0.5">{form.activo ? 'En operacion' : 'Dado de baja'}</p>
                </button>
              </div>
            </div>
          </div>
